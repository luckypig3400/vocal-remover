...
class VocalRemoverTrainingSet(torch.utils.data.Dataset):
    ...
    def __init__(self, training_set, cropsize, n_fft, reduction_rate, reduction_weight, mixup_rate, mixup_alpha):
    ...
    def __getitem__(self, idx):
        ...
        X_arr = np.concatenate([np.abs(X), np.angle(X)])
        y_arr = np.concatenate([np.abs(y), np.angle(y)])
