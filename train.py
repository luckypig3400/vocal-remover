...
def train_epoch(dataloader, model, device, optimizer):
    ...
    for X_batch, y_batch in dataloader:
        ...
        model.zero_grad()
        pred, _, aux = model(X_batch)

        loss_main = crit(pred * X_batch[:, :2], y_batch[:, :2])
        loss_aux = crit(aux * X_batch[:, :2], y_batch[:, :2])
...
def train_phase_epoch(dataloader, model, device, optimizer):
    sum_loss = 0
    ...
    for X_batch, y_batch in dataloader:
        ...
        ew_loss = crit(pred * X_batch[:, 2:], y_batch[:, 2:])
        ew_loss *= X_batch[:, :2]
        loss = torch.mean(ew_loss)
...
def validate_epoch(dataloader, model, device):
    ...
    with torch.no_grad():
        for X_batch, y_batch in dataloader:
            ...
            pred, _ = model.predict(X_batch)

            y_batch = spec_utils.crop_center(y_batch, pred)
            loss = crit(pred, y_batch[:, :2])
...
def validate_phase_epoch(dataloader, model, device):
    sum_loss = 0
    crit = nn.L1Loss(reduction='none')
    ...
    with torch.no_grad():
        for X_batch, y_batch in dataloader:
            ...
            _, pred = model.predict(X_batch)

            X_batch = spec_utils.crop_center(X_batch, pred)
            y_batch = spec_utils.crop_center(y_batch, pred)
            ew_loss = crit(pred, y_batch[:, 2:])
            ew_loss *= X_batch[:, :2]
            loss = torch.mean(ew_loss)
...
def main():
    ...
    model = nets.CascadedNet(args.n_fft, predict_mag=not args.phase, predict_phase=args.phase)
    ...
    for epoch in range(args.epoch):
        ...
        for m in freeze_module_list:
            m.eval()
        if args.phase:
            train_loss = train_phase_epoch(train_dataloader, model, device, optimizer)
        else:
            train_loss = train_epoch(train_dataloader, model, device, optimizer)

        model.eval()
        if args.phase:
            val_loss = validate_phase_epoch(val_dataloader, model, device)
        else:
